...
import { Check, Circle, CircleCheck, Recycle, LogOut, Settings, ArrowLeft, House } from 'lucide-react';
import './index.css';
...
export default function App() {
  ...
  async function refresh() { const data = await request('/weeks'); setWeeks(data); }
  useEffect(() => {
    function expired() { setUser(null); setWeeks([]); setAdmin(false); csrf = null; }
  ...
  useEffect(() => {
    if (!user) return;
    let active = true;
    const load = () => request('/weeks').then(data => { if (active) {setWeeks(data); setError('');} }).catch(err => {if (active) setError(err.message);});
  ...
  async function complete(task) {
    ...
    try {await request(`/weeks/${current.start}/tasks/${task.id}/completion`, {method:'POST'}); await refresh();}
